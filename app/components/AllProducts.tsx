...
const AllProducts = async () => {
    ...
    return (
        <div className='w-[80%]  grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-10 gap-x-3 mb-20  mx-auto mt-16'>
            {products.map((product) =>
                <Link href={`/products/${product.id}`} key={product.id}>
                    <div className='flex hover:scale-105 hover:drop-shadow-lg transition-all flex-col items-center  space-y-1'>

                        <div className='relative md:w-48 flex outline-2  outline-slate-100 flex-col space-y-1 justify-center items-center md:h-48 w-32 h-32 border-2 border-yellow-700/60 p-2'
                            style={{
